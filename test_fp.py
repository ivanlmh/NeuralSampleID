...
parser.add_argument(
    "--config", default="config/grafp.yaml", type=str, help="Path to config file"
)
parser.add_argument("--test_config", default="config/test_config.yaml", type=str)
parser.add_argument(
    "--seed", default=42, type=int, help="seed for initializing testing. "
)
parser.add_argument(
    "--test_dir", default="data/fma_medium.json", type=str, help="path to test data"
)
...
def create_fp_db(dataloader, augment, model, output_root_dir, verbose=True):
    ...
    for idx, (audio, aux_audio, metadata) in enumerate(dataloader):
        audio = audio.to(device)
        x_i, x_j, metadata = augment(audio, aux_audio, metadata)
        with torch.no_grad():
    ...
    arr_db[:] = fp_db[:]
    arr_db.flush()
    del arr_db  # Close memmap

    np.save(f"{output_root_dir}/db_shape.npy", arr_shape)


def create_dummy_db(
    dataloader, augment, model, output_root_dir, fname="dummy_db", verbose=True
):
    fp = []
    print("=> Creating dummy fingerprints...")
    for idx, (audio, aux_audio, metadata) in enumerate(dataloader):
        audio = audio.to(device)
        x_i, _, aug_metadata = augment(audio, aux_audio, metadata)
    ...
    arr = np.memmap(
        f"{output_root_dir}/{fname}.mm", dtype="float32", mode="w+", shape=arr_shape
    )
    arr[:] = fp[:]
    arr.flush()
        ...
        for epoch in epochs:
            ...
            if args.query_lens is not None:
                ...
                writer.add_text(
                    "table",
                    create_table(
                        hit_rates, cfg["overlap"], cfg["dur"], test_seq_len, text=text
                    ),
                    label,
                )
                ...
                writer.add_text(
                    "table",
                    create_table(hit_rates, cfg["overlap"], cfg["dur"], text=text),
                    label,
                )
